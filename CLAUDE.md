...
## 自动化协作与 Git 提交准则 (Git & Agent Workflow Guidelines)

- **修改完成必须自动分批 Git 提交 (Mandatory Auto Batch Commits)**:
  - **任务完成自动提交**：每次开发、重构或 Bug 修复任务在通过验证后，**必须主动、自动执行 Git 提交**，无需等待用户额外提醒。
  - **核心铁律**：**严禁堆积多个模块或一次性提交一个巨大的变更（No giant commits）**！
