...
import type { SearchResult, CityKnowledge } from './rag'
import type { WeatherData } from './weather'

// @ts-expect-next-line -- import attributes 语法兼容
import attractionsDB from '../../knowledge/attractions.json' assert { type: 'json' }
import type { SSEData } from '../utils/sse'
import { createLogger } from '../utils/logger'

const log = createLogger('agent')
import { matchAttractionRefsFromItinerary } from './attractions/attractionMatcher'
import { callLLMWithTools, getLLMConfig } from './llm'
...
async function executeAgent(send: (data: SSEData) => void, params: AgentParams): Promise<void> {
  const config = getLLMConfig()

  if (!config) {
    return executeAgentMock(send, params)
  }
  ...
  try {
    for (let round = 0; round < 10; round++) {
      ...
      for (const toolCall of assistantMsg.tool_calls) {
        ...
        if (stepNum) {
          send( { type: 'step', step: stepNum, name: STEP_NAMES[stepNum], status: 'start' })
        }

        const result = await executeTool(name, args, context)

        if (stepNum) {
          send( { type: 'step', step: stepNum, name: STEP_NAMES[stepNum], status: 'complete', data: result.summary })
        }
    ...
    // 补执行缺失步骤
    if (!context.intent) {
      send( { type: 'step', step: 1, name: '解析意图', status: 'start' })
      const result = await executeTool('parse_intent', params as unknown as Record<string, unknown>, context)
      send( { type: 'step', step: 1, name: '解析意图', status: 'complete', data: result.summary })
    }
    if (!context.ragResult) {
      send( { type: 'step', step: 2, name: '知识库检索', status: 'start' })
      const result = await executeTool('search_attractions', { city: context.intent?.city || params.city }, context)
      send( { type: 'step', step: 2, name: '知识库检索', status: 'complete', data: result.summary })
    }
    if (!context.weather) {
      send( { type: 'step', step: 3, name: '查询天气', status: 'start' })
      context.weather = await getWeather(params.city).catch(() => null)
      send( { type: 'step', step: 3, name: '查询天气', status: 'complete', data: context.weather ? { temperature: context.weather.temperature, weatherDesc: context.weather.weatherDesc } : {} })
    }

    if (!context.itinerary || !context.itinerary.length) {
      send( { type: 'step', step: 4, name: '行程规划', status: 'start' })
      const itinerary = buildWeatherAwareItinerary(context)
      context.itinerary = itinerary
      send( { type: 'step', step: 4, name: '行程规划', status: 'complete', data: { days: itinerary.length, spotCount: itinerary.length * 3 } })
    }

    if (!context.budgetBreakdown) {
      send( { type: 'step', step: 5, name: '预算计算', status: 'start' })
      ...
      send( { type: 'step', step: 5, name: '预算计算', status: 'complete', data: result.summary })
    }

    if (!context.tips) {
      send( { type: 'step', step: 6, name: '生成建议', status: 'start' })
      ...
      send( { type: 'step', step: 6, name: '生成建议', status: 'complete', data: result.summary })
    }
...
async function executeAgentMock(send: (data: SSEData) => void, params: AgentParams): Promise<void> {
  // 步骤 1：解析意图
  send( { type: 'step', step: 1, name: '解析意图', status: 'start' })
  const tags: string[] = []
  ...
  send( { type: 'step', step: 1, name: '解析意图', status: 'complete', data: { city: intent.city, days: intent.days, budget: intent.budget, tags: intent.tags } })

  // 步骤 2：检索景点
  send( { type: 'step', step: 2, name: '知识库检索', status: 'start' })
  const ragResult = await retrieve(intent.city, intent.tags, intent.city)
  if (!ragResult) {
    send( { type: 'complete', data: { dailyItinerary: [], budgetBreakdown: null, tips: ['暂不支持该城市的行程规划'] } })
    return
  }
  send( { type: 'step', step: 2, name: '知识库检索', status: 'complete', data: { count: ragResult.attractions.length, sources: ragResult.attractions.slice(0, 5).map(a => a.name) } })

  // 步骤 3：查询天气
  send( { type: 'step', step: 3, name: '查询天气', status: 'start' })
  const weather = await getWeather(intent.city).catch(() => null)
  send( { type: 'step', step: 3, name: '查询天气', status: 'complete', data: weather ? { temperature: weather.temperature, weatherDesc: weather.weatherDesc } : {} })
  ...
  send( { type: 'step', step: 4, name: '行程规划', status: 'complete', data: { days: intent.days, spotCount: intent.days * 3 } })
  ...
  send( { type: 'step', step: 5, name: '预算计算', status: 'complete', data: budgetBreakdown })
  ...
  if (intent.days >= 3)
    tips.push('行程较长，建议准备舒适的运动鞋')
  tips.push('建议提前在网上预约热门景点门票')
  send( { type: 'step', step: 6, name: '生成建议', status: 'complete', data: { count: tips.length } })
