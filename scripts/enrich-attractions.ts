 ...
 */
import type { AttractionBookingLinks, AttractionTicketType } from '../src/types/attraction'
...
export function generateAttractionId(city: string, name: string): string {
  ...
  const citySlug = pinyinMap[cleanCity] || cleanCity.replace(/\s+/g, '-').replace(/[^\w\u4E00-\u9FA5-]+/g, '')
  const nameSlug = pinyinMap[cleanName] || cleanName.replace(/\s+/g, '-').replace(/[^\w\u4E00-\u9FA5-]+/g, '')
...
export function generateMockEnrichedEntry(
  ...
  const isFree = overrides.ticketType === 'free' || name.includes('街') || name.includes('公园') || name.includes('巷子')
  const ticketType: AttractionTicketType = isFree ? 'free' : 'paid'
  const priceText = isFree ? '免费开放，以官方公告为准' : '参考价 ¥30-80，以平台实际为准'
  ...
  return {
    ...
    city,
    coverImage: overrides.coverImage || `/images/attractions/${city}/${id}.webp`,
    description: overrides.description || `${name}是${city}的代表性地标，融合自然风光与历史人文底蕴，适合列入深度游览路线。`,
    ...
    id,
    name,
    openingHours: overrides.openingHours || '全天开放（部分内部场馆09:00-17:00）',
    ...
    summary: overrides.summary || `${name}是${city}必去胜地，兼具文化底蕴与手账打卡价值。`,
 ...
 * 校验生成的数据是否满足系统入库规范
 */
export function validateEnrichedAttraction(entry: EnrichedAttractionPayload): { valid: boolean, errors: string[] } {
  const errors: string[] = []
  if (!entry.id) errors.push('缺少 id')
  if (!entry.name) errors.push('缺少 name')
  if (!entry.city) errors.push('缺少 city')
  if (!['free', 'paid'].includes(entry.ticketType)) errors.push('ticketType 必须为 free 或 paid')
  if (!Array.isArray(entry.highlights) || entry.highlights.length === 0) errors.push('highlights 至少提供 1 条')
  if (!Array.isArray(entry.tips) || entry.tips.length === 0) errors.push('tips 至少提供 1 条')
  if (!Array.isArray(entry.suitableFor) || entry.suitableFor.length === 0) errors.push('suitableFor 至少提供 1 条')
...
export async function main(args: string[] = process.argv.slice(2)): Promise<void> {
  ...
  console.log(`[enrich-attractions] 启动景点自动化补全工具 (dry-run: ${isDryRun})`)
  ...
  for (const item of results) {
    ...
    if (!valid) {
      ...
      console.log(`✓ 成功生成并校验: [${item.city}] ${item.name} (${item.id}) -> ${item.bookingLinks.ctrip}`)
    }
  }
