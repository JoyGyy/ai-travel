 ...
 */
import type { ChatMessage, ToolDefinition } from '@/lib/services/llm'
import type { SearchResult } from '@/lib/services/rag'
...
import { callLLMWithTools, getLLMConfig } from '@/lib/services/llm'
...
const TRAVEL_KEYWORDS = [
  '旅行', '旅游', '出行', '自由行', '跟团', '自驾', '攻略', '行程', '路线',
  '规划', '目的地', '城市', '景点', '门票', '游玩', '打卡', '美食', '餐厅',
  '小吃', '酒店', '住宿', '民宿', '交通', '地铁', '公交', '高铁', '火车',
  '机票', '机场', '预算', '费用', '花费', '签证', '护照', '保险', '安全',
  '天气', '季节', '亲子游', '情侣游', '毕业旅行', '蜜月', '周末游', '夜游',
  '避暑', '周边游',
]

const NON_TRAVEL_KEYWORDS = [
  '代码', '编程', '函数', '组件', '接口', '数据库', '算法', '数学题',
  '论文', '股票', '基金', '理财', '政治', '简历', '翻译', '作文',
  'react', 'vue', 'python', 'java', 'typescript', 'javascript', 'css', 'html',
]
...
async function streamTextResponse(
  ...
  for (const char of content) {
    accumulated += char
    send({ type: 'chunk', content: char })
    if (delayMs > 0 && (char === '\n' || accumulated.length % 5 === 0)) {
      await delay(delayMs)
    }
  }

  send({ type: 'complete', data: { content, sources } })
}
...
async function handleNonTravelMessage(
  ...
  await streamTextResponse(send, getNonTravelResponse(), { sources: [], delayMs: options.delayMs ?? 10 })
  return true
}
...
async function executeChatTool(toolName: string, args: Record<string, unknown>): Promise<string> {
  switch (toolName) {
    ...
    case 'search_product_attractions':
      return await executeProductAttractionsTool(args as ProductAttractionsArgs)
    default:
...
async function executeProductAttractionsTool(args: ProductAttractionsArgs): Promise<string> {
  const filters = {
  ...
  const attractions = (await searchAttractions(filters) as unknown as Array<{
    id: string
    name: string
    city: string
    ticketType: string
...
export async function POST(req: Request) {
  try {
    ...
    return createSSEStream(async (send) => {
      ...
      if (await handleNonTravelMessage(message, send)) return
        ...
        try {
          ...
          for (let round = 0; round < 5; round++) {
            ...
            for (const toolCall of assistantMsg.tool_calls) {
              ...
              // 发送 step start 事件
              send({ type: 'step', step: stepCounter, name: stepName, status: 'start' })
              ...
              // 发送 step complete 事件
              send({ type: 'step', step: stepCounter, name: stepName, status: 'complete', data: summary })
          ...
          if (finalContent) {
            for (let i = 0; i < finalContent.length; i++) {
              send({ type: 'chunk', content: finalContent[i] })
              if (finalContent[i] === '\n' || i % 5 === 0) {
                await delay(15)
              }
            }

            send({ type: 'complete', data: { content: finalContent, sources: ragSources } })
            return
          }
        }
        catch (err: unknown) {
          log.error('Chat Agent 失败，降级到 Mock:', (err as Error).message)
          send({ type: 'notice', message: 'AI 模型暂不可用，已切换为知识库模式' })
        }
      }
      ...
      if (ragResult) {
        stepCounter++
        send({ type: 'step', step: stepCounter, name: '查询知识库', status: 'start' })
        await delay(100)
        send({ type: 'step', step: stepCounter, name: '查询知识库', status: 'complete', data: { city: ragResult.city, attractionCount: ragResult.attractions.length } })
      }
      ...
      send({ type: 'complete', data: { content: mockReply, sources: ragSources } })
    })
  }
  catch (err) {
