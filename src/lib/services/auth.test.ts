...
describe('auth 服务', () => {
  ...
  describe('register()', () => {
    ...
    it('用户名过短时抛出错误', async () => {
      await expect(register('a', 'Password123')).rejects.toThrow(
        '用户名长度为 2-20 个字符',
      );
    });
    ...
    it('密码过短时抛出错误', async () => {
      await expect(register('testuser', 'Ab1')).rejects.toThrow(
        '密码长度至少 8 个字符',
      );
    });
  ...
  describe('login()', () => {
    ...
    it('支持使用注册邮箱成功登录', async () => {
      ...
      // act
      const result = await login('myemail@test.com', 'Password123');

      // assert
      expect(result.token).toBe('login-token');
      expect(result.user.username).toBe('testuser');
      expect(mockQuery).toHaveBeenCalledWith(
        expect.stringContaining('username = $1 OR LOWER(email) = LOWER($1)'),
        ['myemail@test.com'],
      );
    });
    ...
    it('密码错误时抛出错误', async () => {
      ...
      // act & assert
      await expect(login('testuser', 'WrongPassword1')).rejects.toThrow(
        '用户名或密码错误',
      );
    });
  });
  ...
  describe('getAuthFromHeaders()', () => {
    it('优先从 Authorization header (Bearer Token) 认证用户', async () => {
      // arrange
      const payload = { id: 'user-1', username: 'testuser' };
      mockJwtVerify.mockResolvedValue({ payload });
    ...
    it('不误匹配 csrf_token 等类似 cookie 名', async () => {
      ...
      mockJwtVerify.mockResolvedValue({ payload });
      const headers = new Headers({
        cookie: 'csrf_token=abc123; token=real-token',
      });
    ...
    it('不误匹配带连字符的 cookie 名如 app-token 或 other-token', async () => {
      ...
      mockJwtVerify.mockResolvedValue({ payload });
      const headers = new Headers({
        cookie: 'app-token=fake123; token=real-token',
      });
  ...
  describe('consumeAiQuota()', () => {
    ...
    it('达到上限时抛出 429 错误', async () => {
      ...
      // act & assert
      const err = (await consumeAiQuota('user-1', '2024-01-15').catch(
        (e) => e,
      )) as Error & {
        status: number;
      };
  ...
  describe('addFavoriteAttraction()', () => {
    it('成功添加收藏', async () => {
      ...
      // act & assert
      await expect(
        addFavoriteAttraction('user-1', 'attraction-1'),
      ).resolves.toBeUndefined();
    });

    it('userId 为空时抛出错误', async () => {
      await expect(addFavoriteAttraction('', 'attraction-1')).rejects.toThrow(
        '用户信息无效',
      );
    });

    it('attractionId 为空时抛出错误', async () => {
      await expect(addFavoriteAttraction('user-1', '')).rejects.toThrow(
        '景点信息无效',
      );
    });
  });
  ...
  describe('removeFavoriteAttraction()', () => {
    it('成功取消收藏', async () => {
      ...
      // act & assert
      await expect(
        removeFavoriteAttraction('user-1', 'attraction-1'),
      ).resolves.toBeUndefined();
    });

    it('userId 为空时抛出错误', async () => {
      await expect(
        removeFavoriteAttraction('', 'attraction-1'),
      ).rejects.toThrow('用户信息无效');
    });

    it('attractionId 为空时抛出错误', async () => {
      await expect(removeFavoriteAttraction('user-1', '')).rejects.toThrow(
        '景点信息无效',
      );
    });
  });
  ...
  describe('changePassword()', () => {
    ...
    it('当前密码错误时抛出错误', async () => {
      ...
      // act & assert
      await expect(
        changePassword('user-1', 'WrongPassword1', 'NewPassword1'),
    ...
    it('用户不存在时抛出错误', async () => {
      ...
      // act & assert
      await expect(
        changePassword('nonexistent', 'OldPassword1', 'NewPassword1'),
    ...
    it('当前密码为空时抛出错误', async () => {
      await expect(
        changePassword('user-1', '', 'NewPassword1'),
      ).rejects.toThrow('当前密码和新密码不能为空');
    });

    it('新密码为空时抛出错误', async () => {
      await expect(
        changePassword('user-1', 'OldPassword1', ''),
      ).rejects.toThrow('当前密码和新密码不能为空');
    });

    it('新密码过短时抛出错误', async () => {
      await expect(
        changePassword('user-1', 'OldPassword1', 'Ab1'),
      ).rejects.toThrow('密码长度至少 8 个字符');
    });
  });
});
