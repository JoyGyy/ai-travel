  ...
  return (
    <main className="community-page travel-page-shell" aria-labelledby="community-title">
      <section className="community-page__hero travel-page-hero travel-ticket-edge travel-route-line">
        ...
        <Button type="primary" size="large" icon={<PlusOutlined aria-hidden="true" />} onClick={() => (requireLogin('发布分享') ? navigate('/community/new') : undefined)}>
          发布旅行分享
        </Button>
      </section>
      ...
      {!loading && !error && items.length === 0
        ? (
            <div className="community-page__state travel-surface-card">
              ...
              <Button type="primary" onClick={() => (requireLogin('发布分享') ? navigate('/community/new') : undefined)}>发布第一条分享</Button>
            </div>
          )
        : null}
              ...
              <section className="community-page__feed" aria-label="社区分享列表">
                {items.map(post => (
                  <CommunityPostCard
                    ...
                    onLike={toggleLike}
                    onComment={item => navigate(`/community/${item.id}`)}
                    onRepost={openRepost}
                  />
                ))}
              </section>
