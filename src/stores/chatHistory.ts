...
function getStorageKey(userId: string | null): string {
  return userId
    ? `travel_chat_sessions_${userId}`
    : 'travel_chat_sessions_guest';
}
    ...
    if (userId) {
      ...
      if (oldRaw) {
        try {
          ...
          const sessions = Array.isArray(parsedOld?.state?.sessions)
            ? parsedOld.state.sessions
            : [];
...
function areMessagesEqual(a: UIMessage[], b: UIMessage[]): boolean {
  ...
  for (let i = 0; i < a.length; i++) {
    const ma = a[i];
    const mb = b[i];
    if (ma.id !== mb.id || ma.role !== mb.role) return false;
    ...
    if (ma.parts && mb.parts) {
      for (let j = 0; j < ma.parts.length; j++) {
        ...
        if (pa.type !== pb.type || pa.text !== pb.text) return false;
      }
    }
  }
  return true;
    ...
    (set, get) => ({
      ...
      saveMessages: (id, messages, detectedCity) => {
        ...
        if (!existing) {
          ...
          if (
            isSameMessages &&
            (!detectedCity || existing.city === detectedCity)
          ) {
            return;
          }
          ...
          const shouldUpdateTitle =
            existing.title === '新的手账对话' || existing.title === '新建对话';
        ...
        activeSyncPromise = (async () => {
          ...
          if (res?.sessions) {
            ...
            set((state) => {
              ...
              if (
                mapped.length > 0 &&
                (!nextActiveId || !mapped.some((s) => s.id === nextActiveId))
              ) {
