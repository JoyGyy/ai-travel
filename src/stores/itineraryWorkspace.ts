...
export type ResourceCategory = 'attractions' | 'hotels' | 'restaurants' | 'shopping' | 'transport'
...
export interface ItineraryWorkspaceState {
  ...
  addSpotToDay: (dayNum: number, spot: Partial<WorkspaceSpotNode> & { name: string }) => void
  city: string
  clearWorkspace: () => void
  days: ItineraryDayPlan[]
  initFromParsedRoute: (data: ParsedRouteData, promptTitle?: string) => void
  isGenerating: boolean
  moveSpotBetweenDays: (fromDay: number, fromIndex: number, toDay: number, toIndex: number) => void
 ...
 */
function recalculateLegs(spots: WorkspaceSpotNode[], mode: 'driving' | 'transit' | 'walking'): WorkspaceLeg[] {
  if (spots.length < 2)
    return []
...
function buildSpotNode(
  ...
  return {
    ...
    id: overrides.id || `${info.id}-${idx}-${Date.now()}`,
    ...
    recommendedDuration: overrides.recommendedDuration || info.recommendedDuration,
  ...
  devtools(
    immer(set => ({
      ...
      addFromUnassignedToDay: (unassignedIndex, toDay) =>
        set((state) => {
          if (state.unassignedSpots[unassignedIndex] === undefined)
            return
          ...
          targetDay.legs = recalculateLegs(targetDay.spots, state.transportMode)
        }),
      ...
      addSpotToDay: (dayNum, spot) =>
        set((state) => {
          ...
          const newSpot = buildSpotNode(spot.name, state.city, targetDay.spots.length, spot)
            ...
            data.days.forEach((d, dayIdx) => {
              const daySpots: WorkspaceSpotNode[] = d.spots.map((s, spotIdx) =>
                buildSpotNode(s.name, data.city, spotIdx, {
                  ...
                  id: s.id ? `${s.id}-d${d.day}-${spotIdx}` : `spot-${d.day}-${spotIdx}`,
      ...
      moveSpotBetweenDays: (fromDay, fromIndex, toDay, toIndex) =>
        set((state) => {
          ...
          if (!srcDay || !destDay)
            return
          if (srcDay.spots[fromIndex] === undefined)
            return
      ...
      moveSpotInDay: (dayNum, fromIndex, toIndex) =>
        set((state) => {
          ...
          if (!day || day.spots[fromIndex] === undefined)
            return
      ...
      moveToUnassigned: (dayNum, spotIndex) =>
        set((state) => {
          const day = state.days.find(d => d.day === dayNum)
          if (!day || day.spots[spotIndex] === undefined)
            return
      ...
      removeDay: dayNum =>
        set((state) => {
          ...
          state.selectedDay = Math.min(state.selectedDay, Math.max(1, state.days.length))
        }),
