...
describe('storage loadItineraryCache / saveItineraryCache', () => {
  ...
  it('写入并读取合法的缓存数据', () => {
    const cacheData: ItineraryCache = {
      ...
      itinerary: [
        {
          day: 1,
          morning: { description: '参观故宫博物院', duration: '3小时', spot: '故宫' },
          spots: [{ description: '参观故宫博物院', duration: '3小时', name: '故宫' }],
          title: '故宫深度一日游',
        },
      ],
      nightlife: ['三里屯'],
  ...
  it('当缓存结构不符合 Schema（缺少必需字段 itinerary）时安全返回 null 并清理旧缓存', () => {
    ...
    localStorage.setItem(key, JSON.stringify({ city: '成都', tips: ['吃火锅'] }))
